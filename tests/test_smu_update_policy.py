...
class TestSmuWorktreeIgnores(unittest.TestCase):
    ...
    def test_local_init_creates_ignored_paths_without_dirtying_blueprint(self):
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            with patch.object(smu, "smu_home_dir", blueprint), \
                    patch.object(smu, "config_dir", config_dir), \
                    patch.object(smu, "local_config_path", os.path.join(config_dir, "local.env")), \
                    patch.object(smu, "local_dotfiles_dir", os.path.join(blueprint, "dotfiles", "local")), \
                    patch.object(smu, "rcrc", os.path.join(blueprint, "dotfiles", "rcrc")):
